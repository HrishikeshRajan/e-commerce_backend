...
router.route('/comment')
    .post(auth_1.isLoggedIn, (0, userInputValidator_1.validateRequest)({ body: user_schemaTypes_1.PhotoSchema }), image_helper_1.multerUpload);
//Is loggedIn 
...
router.get('/authStatus/checkToken', (req, res) => {
    Logger_1.default.info('Auth status checking initiated');
    const token = (req.cookies) ? req.cookies.token : null;
    Logger_1.default.info(`Cookies found, sending success response`);
    if (!token) {
        Logger_1.default.error(`Cookies not found, sending error response`);
